...
| 通过选择最相关的特征来减少数据集的维度，可以使用各种特征选择技术，例如方差阈值、相关系数、信息增益等。
| L1正则化（Lasso）可以推动模型将一些特征的权重归零，从而实现特征选择。

2.主成分分析 (Principal Component Analysis, PCA):

| PCA是一种降维技术，可以将高维数据映射到低维空间，保留最重要的特征。
| 通过保留主成分，可以减少数据的维度，同时尽量保留原始数据的方差。
...
| RFE是一种递归的特征选择方法，获取每个特征的重要程度，剔除最不重要的特征，不断的重复递归这个步骤，直到达到所需的特征数量。
| 常用于对模型的复杂度进行控制，同时选择最相关的特征。
...
7.稳定性选择 (Stability Selection):

| 通过在不同的子样本上运行特征选择算法，可以提高特征选择的稳定性。


8.方差阈值 (Variance Threshold):
